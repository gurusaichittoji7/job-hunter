...
def run():
    ...
    print("\n[1/9] Fetching jobs across all roles...")
    jsearch_jobs = fetch_all_jobs()
    print(f"      → {len(jsearch_jobs)} jobs fetched from JSearch")

    print("\n[2/9] Checking target companies' direct job boards...")
    ...
    print(f"\n[6/9] Checking for direct company apply links...")
    enriched_jobs = enrich_all(new_jobs)
    print("      → enrichment complete")

    print(f"\n[7/9] Scoring {len(enriched_jobs)} jobs against your resume...")
    scored = score_jobs(enriched_jobs)
    print("      → scoring complete")

    print("\n[8/9] Selecting top 50 (75%+, max 2/company)...")
    top_jobs = select_top_jobs(scored)
    print(f"      → {len(top_jobs)} jobs made the final cut")

    print("\n[9/9] Generating report and sending email...")
